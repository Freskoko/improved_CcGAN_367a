...
if args.GAN == "CcGAN":
    ...
    counts_tmp = unique_counts_norm[indx_tmp].reshape(-1,1)
    counts_tmp = torch.from_numpy(counts_tmp).type(torch.float).to(device)
    epsilons_tmp = np.random.normal(0, 0.2, len(counts_tmp))
    epsilons_tmp = torch.from_numpy(epsilons_tmp).view(-1,1).type(torch.float).to(device)
    counts_tmp = torch.clamp(counts_tmp+epsilons_tmp, 0.0, 1.0)
    net_embed.eval()
    net_h2y = net_embed.h2y
    net_y2h.eval()
    with torch.no_grad():
        counts_rec_tmp = net_h2y(net_y2h(counts_tmp)).cpu().numpy().reshape(-1,1)
    results = np.concatenate((counts_tmp.cpu().numpy(), counts_rec_tmp), axis=1)
...
# Concitnuous cGAN
    ...
    if not os.path.isfile(Filename_GAN):
        netG = cont_cond_cnn_generator(nz=args.dim_gan, dim_embed=args.dim_embed)
        netD = cont_cond_cnn_discriminator(dim_embed=args.dim_embed)
        netG = nn.DataParallel(netG)
        netD = nn.DataParallel(netD)

        # Start training
        netG, netD = train_CcGAN(args.kernel_sigma, args.kappa, images, counts, netG, netD, net_y2h, save_images_folder=save_GANimages_InTrain_folder, save_models_folder = save_models_folder)
    ...
    def fn_sampleGAN_given_label(nfake, label, batch_size):
        fake_images, fake_counts = SampCcGAN_given_label(netG, net_y2h, label, path=None, NFAKE = nfake, batch_size = batch_size)
        return fake_images, fake_counts
