...
class cont_cond_cnn_generator(nn.Module):
    def __init__(self, nz=128, ngf=64, nc=NC, bias = default_bias):
        ...
        self.main = nn.Sequential(
            # nn.Conv2d(ngf*8, ngf*8, 3, stride=1, padding=1, bias=bias),  #h=h
            # nn.BatchNorm2d(ngf*8),
            # nn.ReLU(True),
            ...
            nn.ReLU(True),
            # nn.Conv2d(ngf*4, ngf*4, 3, stride=1, padding=1, bias=bias),  #h=h
            # nn.BatchNorm2d(ngf*4),
            # nn.ReLU(True),
            # state size. 16 x 16
            ...
            nn.ReLU(True),
            # nn.Conv2d(ngf*2, ngf*2, 3, stride=1, padding=1, bias=bias),  #h=h
            # nn.BatchNorm2d(ngf*2),
            # nn.ReLU(True),
            # state size. 32 x 32
...
class cont_cond_cnn_discriminator(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # output = output.view(-1, self.ndf*8*4*4) + y.repeat(1,self.ndf*8*4*4)
