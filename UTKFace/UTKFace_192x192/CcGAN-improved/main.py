...
if args.visualize_fake_images:
    ...
    if args.GAN == "CcGAN":
        ...
        filename_continous_fake_images = os.path.join(save_images_folder, 'fake_images_grid_continuous_fake_images_grid.png')
    ...
    if not os.path.isfile(filename_real_images):
        ...
        for i_row in range(n_row):
            ...
            for j_col in range(n_col):
                ...
                sel_indx = indx_curr_label[0]
                while sel_indx in visited:
                    np.random.shuffle(indx_curr_label)
                    sel_indx = indx_curr_label[0]
                visited.add(sel_indx)
                images_show[i_row*n_col+j_col] = raw_images[sel_indx]
        images_show = (images_show/255.0-0.5)/0.5
        images_show = torch.from_numpy(images_show)
