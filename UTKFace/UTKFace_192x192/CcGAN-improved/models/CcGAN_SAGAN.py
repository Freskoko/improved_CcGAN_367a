...
class CcGAN_SAGAN_Generator(nn.Module):
    ...
    def __init__(self, dim_z, dim_embed=128, nc=3, gene_ch=64):
        ...
        self.label_encoder = nn.Sequential(
            nn.Linear(dim_embed, dim_embed, bias=True),
            nn.ReLU()
            ) #accutally not used

        self.snlinear0 = snlinear(in_features=dim_z, out_features=gene_ch*16*6*6)
    ...
    def forward(self, z, labels):
        ...
        # n x dim_z
        out = self.snlinear0(z)            # 6*6
        out = out.view(-1, self.gene_ch*16, 6, 6) # 6 x 6
        out = self.block1(out, labels)    # 12 x 12
        out = self.block2(out, labels)    # 24 x 24
        out = self.block3(out, labels)    # 48 x 48
        out = self.self_attn(out)         # 48 x 48
        out = self.block4(out, labels)    # 96 x 96
        out = self.block5(out, labels)    # 192 x 192
        out = self.bn(out)
        out = self.relu(out)
...
class CcGAN_SAGAN_Discriminator(nn.Module):
    ...
    def __init__(self, dim_embed=128, nc=3, disc_ch=64):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.snlinear1 = snlinear(in_features=disc_ch*16*6*6, out_features=1)
        self.sn_embedding1 = snlinear(dim_embed, disc_ch*16*6*6, bias=False)
    ...
    def forward(self, x, labels):
        ...
        out = self.opt_block1(x)  # 96 x 96
        out = self.block1(out)    # 48 x 48
        out = self.self_attn(out) # 48 x 48
        out = self.block2(out)    # 24 x 24
        out = self.block3(out)    # 12 x 12
        out = self.block4(out)    # 6 x 6
        out = self.block5(out, downsample=False)    # 6 x 6
        out = self.relu(out)              # n x disc_ch*16 x 6 x 6
        out = out.view(-1, self.disc_ch*16*6*6)
