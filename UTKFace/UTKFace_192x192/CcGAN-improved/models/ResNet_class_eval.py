...
class ResNet_class_eval(nn.Module):
    def __init__(self, block, num_blocks, num_classes=200, nc=NC, ngpu = 1):
        ...
        self.main = nn.Sequential(
            ...
            self._make_layer(block, 128, num_blocks[1], stride=2), # h=h/2 24
            self._make_layer(block, 256, num_blocks[2], stride=2), # h=h/2 12
            self._make_layer(block, 512, num_blocks[3], stride=2), # h=h/2 6
