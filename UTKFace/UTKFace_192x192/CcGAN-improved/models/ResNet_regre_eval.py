...
class ResNet_regre_eval(nn.Module):
    def __init__(self, block, num_blocks, nc=NC, ngpu = 1, feature_layer='f3'):
        ...
        self.block1 = nn.Sequential(
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(2,2), #h=h/2 96
            self._make_layer(block, 64, num_blocks[0], stride=2),  # h=h/2 48
        )
        self.block2 = self._make_layer(block, 128, num_blocks[1], stride=2) # h=h/2 24
        self.block3 = self._make_layer(block, 256, num_blocks[2], stride=2) # h=h/2 12
        self.block4 = self._make_layer(block, 512, num_blocks[3], stride=2) # h=h/2 6
