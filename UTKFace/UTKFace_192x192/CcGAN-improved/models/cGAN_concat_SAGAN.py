...
class cGAN_concat_SAGAN_Generator(nn.Module):
    ...
    def __init__(self, z_dim, dim_c=1, g_conv_dim=64):
        ...
        self.snlinear0 = snlinear(in_features=z_dim+dim_c, out_features=g_conv_dim*16*6*6)
    ...
    def forward(self, z, labels):
        # n x z_dim
        act0 = self.snlinear0(torch.cat((z, labels.view(-1,1)),dim=1))            # n x g_conv_dim*16*6*6
        act0 = act0.view(-1, self.g_conv_dim*16, 6, 6) # n x g_conv_dim*16 x 6 x 6
        act1 = self.block1(act0)    # n x g_conv_dim*16 x 12 x 12
        act2 = self.block2(act1)    # n x g_conv_dim*8 x 24 x 24
        act3 = self.block3(act2)    # n x g_conv_dim*4 x 48 x 48
        act3 = self.self_attn(act3)         # n x g_conv_dim*4 x 48 x 48
        act4 = self.block4(act3)    # n x g_conv_dim*2 x 96 x 96
        act5 = self.block5(act4)    # n x g_conv_dim  x 192 x 192
        act5 = self.bn(act5)                # n x g_conv_dim  x 192 x 192
        act5 = self.relu(act5)              # n x g_conv_dim  x 192 x 192
        act6 = self.snconv2d1(act5)         # n x 3 x 192 x 192
        act6 = self.tanh(act6)              # n x 3 x 192 x 192
        return act6
...
class cGAN_concat_SAGAN_Discriminator(nn.Module):
    ...
    def __init__(self, dim_c=1, d_conv_dim=64):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.snlinear1 = snlinear(in_features=d_conv_dim*16*6*6+dim_c, out_features=1)

    def forward(self, x, labels):
        # n x 3 x 192 x 192
        h0 = self.opt_block1(x) # n x d_conv_dim   x 96 x 96
        h1 = self.block1(h0)    # n x d_conv_dim*2 x 48 x 48
        h1 = self.self_attn(h1) # n x d_conv_dim*2 x 48 x 48
        h2 = self.block2(h1)    # n x d_conv_dim*4 x 24 x 24
        h3 = self.block3(h2)    # n x d_conv_dim*8 x 12 x 12
        h4 = self.block4(h3)    # n x d_conv_dim*16 x 6 x  6
        h5 = self.block5(h4, downsample=False)  # n x d_conv_dim*16 x 6 x 6
        out = self.relu(h5)              # n x d_conv_dim*16 x 6 x 6
        out = out.view(-1,self.d_conv_dim*16*6*6)
