...
class encoder(nn.Module):
    def __init__(self, dim_bottleneck=512, ch=64):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(3, ch, kernel_size=4, stride=2, padding=1), #h=h/2; 96
            nn.BatchNorm2d(ch),
            nn.ReLU(),
            ...
            nn.Conv2d(ch, ch, kernel_size=4, stride=2, padding=1), #h=h/2; 48
            nn.BatchNorm2d(ch),
            nn.ReLU(),
            ...
            nn.Conv2d(ch*2, ch*2, kernel_size=4, stride=2, padding=1), #h=h/2; 24
            nn.BatchNorm2d(ch*2),
            nn.ReLU(),
            ...
            nn.Conv2d(ch*4, ch*4, kernel_size=4, stride=2, padding=1), #h=h/2; 12
            nn.BatchNorm2d(ch*4),
            nn.ReLU(),
            ...
            nn.Conv2d(ch*8, ch*8, kernel_size=4, stride=2, padding=1), #h=h/2; 6
            nn.BatchNorm2d(ch*8),
            nn.ReLU(),
        ...
        self.linear = nn.Sequential(
            nn.Linear(ch*16*6*6, dim_bottleneck),
            # nn.ReLU()
        )

    def forward(self, x):
        feature = self.conv(x)
        feature = feature.view(-1, self.ch*16*6*6)
...
class decoder(nn.Module):
    def __init__(self, dim_bottleneck=512, ch=64):
        ...
        self.linear = nn.Sequential(
            nn.Linear(dim_bottleneck, ch*16*6*6)
        )

        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(ch*16, ch*16, kernel_size=4, stride=2, padding=1), #h=2h; 12
            nn.BatchNorm2d(ch*16),
            nn.ReLU(True),
            ...
            nn.ConvTranspose2d(ch*8, ch*8, kernel_size=4, stride=2, padding=1), #h=2h; 24
            nn.BatchNorm2d(ch*8),
            nn.ReLU(True),
            ...
            nn.ConvTranspose2d(ch*4, ch*4, kernel_size=4, stride=2, padding=1), #h=2h; 48
            nn.BatchNorm2d(ch*4),
            nn.ReLU(True),
            ...
            nn.ConvTranspose2d(ch*2, ch*2, kernel_size=4, stride=2, padding=1), #h=2h; 96
            nn.BatchNorm2d(ch*2),
            nn.ReLU(True),
            ...
            nn.ConvTranspose2d(ch, ch, kernel_size=4, stride=2, padding=1), #h=2h; 192
            nn.BatchNorm2d(ch),
            nn.ReLU(True),
